...
pub enum Code { AccessRequest=1, AccessAccept=2, AccessReject=3 }

impl Code { pub fn from_u8(v:u8)->Option<Self>{ match v{1=>Some(Self::AccessRequest),2=>Some(Self::AccessAccept),3=>Some(Self::AccessReject),_=>None}}}
...
pub enum Attr{ UserName(String), UserPassword(Vec<u8>), VendorSpecific(u32,Vec<u8>), Other(u8,Vec<u8>) }
